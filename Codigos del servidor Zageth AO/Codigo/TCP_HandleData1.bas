    ...
    Select Case UCase$(Left$(rData, 1))
        ...
        Case "-" 'Gritar
            ...
            Exit Sub
              ...
              Case "+"
             If InStr(rData, "°") Then ' esto nose para que sera xD
                Exit Sub
            End If
 If UserList(UserIndex).flags.Muerto = 1 Then ' si esta muerto
 Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Solo los usuarios vivos pueden hablar por global." & FONTTYPE_INFO) ' manda el mensaje
 Exit Sub 'sale del codigo
 End If
 If EstadoGlobal = 0 Then ' si el global esta desactivado
 Call SendData(SendTarget.ToIndex, UserIndex, 0, "||El global esta desactivado." & FONTTYPE_INFO) 'envia msj
 Exit Sub ' se sale codigo
 End If
 If UserList(UserIndex).flags.UGLOBAL = 0 Then ' sitiene descativado
 Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Tienes desactivado el global para activarlo coloca /AGLOBAL" & FONTTYPE_INFO) ' le envia msj
Exit Sub ' se sale code
End If
rData = Right$(rData, Len(rData) - 1)
For LoopC = 1 To LastUser ' se fija de 1 a los user online
If UserList(LoopC).flags.UGLOBAL = 1 Then ' si tienen activado el global
Call SendData(SendTarget.ToAll, 0, 0, "||" & UserList(UserIndex).name & "> " & rData & FONTTYPE_TALK) ' envia el msj a los k tienen activado
End If
            Next LoopC
Exit Sub
        ...
        Case "Ñ" 'Moverse
            ...
            'If UserList(UserIndex).flags.TimesWalk >= 30 Then
                ...
                'If dummy < 6050 Then
                    ...
                    '    Call SendData(SendTarget.ToAdmins, 0, 0, "||Zageth AO> " & UserList(UserIndex).name & " ha sido echado por el servidor por posible uso de SH." & FONTTYPE_SERVER)
                ...
                Else
                    If val(Arg1) <= MAX_INVENTORY_SLOTS And val(Arg1) > 0 Then
                        If UserList(UserIndex).Invent.Object(val(Arg1)).ObjIndex = 0 Then
                                Exit Sub
                        End If
                        Call DropObj(UserIndex, val(Arg1), val(Arg2), UserList(UserIndex).Pos.Map, val(Arg3), val(Arg4))
                    Else
                        Exit Sub
                    End If
                End If

...
If ExpMuChAIaO = 1 Then
Dim ExpIeve
ExpIeve = ExpEVENTO
Npclist(NpcIndex).GiveEXP = val(Leer.GetValue("NPC" & NpcNumber, "GiveEXP")) * ExpIeve
Dim GldIeve
GldIeve = GldEVENTO
Npclist(NpcIndex).GiveGLD = val(Leer.GetValue("NPC" & NpcNumber, "GiveGLD")) * GldIeve
Else
Dim Exp
Exp = ExpMod
Npclist(NpcIndex).GiveEXP = val(Leer.GetValue("NPC" & NpcNumber, "GiveEXP")) * Exp
Dim oro
oro = GldMod
Npclist(NpcIndex).GiveGLD = val(Leer.GetValue("NPC" & NpcNumber, "GiveGLD")) * oro
End If

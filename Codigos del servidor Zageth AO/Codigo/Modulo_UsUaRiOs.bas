...
If UserList(AttackerIndex).Stats.UsuariosMatados < 32000 Then
   If UserList(VictimIndex).Stats.AntiTrucheo <= 40 Then 'Si la victima no tiene antitrucheo mayor a 40, no suma el frag
   Exit Sub
   Else
...
UserList(UserIndex).Stats.AntiTrucheo = 0
End Sub
...
Sub ContarMuerte(ByVal Muerto As Integer, ByVal Atacante As Integer)
    ...
    If Criminal(Muerto) Then
        If UserList(Muerto).Stats.AntiTrucheo <= 40 Then Exit Sub

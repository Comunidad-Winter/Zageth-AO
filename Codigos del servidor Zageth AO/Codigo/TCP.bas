...
If haciendoBK Then
    ...
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Zageth AO> Por favor espera algunos segundos, WorldSave esta ejecutandose." & FONTTYPE_SERVER)
End If

If EnPausa Then
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "BKW")
    Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Zageth AO> Lo sentimos mucho pero el servidor se encuentra actualmente detenido. Intenta ingresar más tarde." & FONTTYPE_SERVER)
End If
...
Call SendData(SendTarget.ToIndex, UserIndex, 0, "LOJJED")
...
#If SeguridadAlkon Then
        ...
        UserList(UserIndex).flags.ValCoDe = Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) & Chr(RandomNumber(65, 90)) 'RandomNumber(20000, 32000))
        UserList(UserIndex).RandomCode = Chr(RandomNumber(1, 255)) & Chr(RandomNumber(1, 255)) & Chr(RandomNumber(1, 255)) & Chr(RandomNumber(1, 255)) & Chr(RandomNumber(1, 255))
        UserList(UserIndex).PrevCheckSum = UserList(UserIndex).RandomCode
        UserList(UserIndex).PacketNumber = 100
        UserList(UserIndex).KeyCrypt = (UserList(UserIndex).RandomCode And 17320) Xor (UserList(UserIndex).flags.ValCoDe Xor 4232)
        '<<<<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "VAL" & UserList(UserIndex).RandomCode)
        Exit Sub
    Else
        '<<<<<<<<<<< MODULO PRIVADO DE CADA IMPLEMENTACION >>>>>>
...
Case "0L0JIN"
 ...
 'aka areglamos el loggin del server
...
If VersionOK(Ver) Then
                    ...
                    If ReadField(11, rData, 44) <> UserList(UserIndex).RandomCode Then
                    ...
                    End If
                Else
                     Call SendData(SendTarget.ToIndex, UserIndex, 0, "ERRVersion Obsoleta")
                     'Call CloseSocket(UserIndex)
                     Exit Sub
                End If
                Exit Sub
...
If UCase$(Left$(rData, 7)) = "/TELEP " Then
    ...
    If UCase$(name) <> "YO" Then
    Call LogGM("EDITADOS", UserList(UserIndex).name & " Transporto a: " & UserList(tIndex).name & " al Mapa:" & UserList(UserIndex).Pos.Map & " X:" & UserList(UserIndex).Pos.X & " Y:" & UserList(UserIndex).Pos.Y, False)
    End If
    Exit Sub
End If
...
If UCase$(Left$(rData, 11)) = "/SILENCIAR " Then
    ...
    Else
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||Usuario DesSilenciado." & FONTTYPE_INFO)
...
If UCase$(Left$(rData, 5)) = "/FPS " Then
        ...
        Else
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||No se encuentra " & rData & FONTTYPE_SERVER)
        Exit Sub
        End If
    End If
...
If UCase$(Left$(rData, 5)) = "/MOD " Then
    ...
    If UserList(tIndex).flags.Privilegios = PlayerType.User Then
    Call SendData(SendTarget.ToAdmins, 0, 0, "||" & UserList(UserIndex).name & " Intento editar el/la " & Arg1 & " a " & UserList(tIndex).name & "." & FONTTYPE_GUILD)
    Call LogGM("EDITADOS", UserList(UserIndex).name & " Intento editar el/la " & Arg1 & " a " & UserList(tIndex).name, False)
    Exit Sub
    End If
...
If UCase$(rData) = "/GLOBAL" Then 'si coloca /global
If UserList(UserIndex).flags.Privilegios > Consejero Then ' y el usuario es mayor rango que consejero
If EstadoGlobal = 1 Then ' ademas si el global esta activado
EstadoGlobal = 0 ' se desactiva
Call WriteVar(IniPath & "Server.ini", "INIT", "Global", EstadoGlobal) ' y se escribe en server.ini en global=0 osea desactivado
Call SendData(SendTarget.ToAll, 0, 0, "||El ah sido desactivado" & FONTTYPE_INFO) ' envia msj
Exit Sub ' se sale code
ElseIf EstadoGlobal = 0 Then ' si esta desactivado
EstadoGlobal = 1 ' se activa
Call WriteVar(IniPath & "Server.ini", "INIT", "Global", EstadoGlobal) 'se coloca escribe en server.ini
Call SendData(SendTarget.ToAll, 0, 0, "||El global a sido activado" & FONTTYPE_INFO) ' se envia msj
Exit Sub ' se sale code
End If
End If ' se termina todos los IF
End If
...
If UCase$(Left$(rData, 5)) = "/BAN " Then
    ...
    If tIndex <= 0 Then
        ...
        If FileExist(CharPath & tStr & ".chr", vbNormal) Then
            ...
            Call SendData(SendTarget.ToAdmins, 0, 0, "||Zageth AO> " & UserList(UserIndex).name & " ha baneado a " & tStr & "." & FONTTYPE_SERVER)
    ...
    Else
        ...
        Call SendData(SendTarget.ToAdmins, 0, 0, "||Zageth AO> " & UserList(UserIndex).name & " ha baneado a " & UserList(tIndex).name & "." & FONTTYPE_SERVER)
...
If UCase$(Left$(rData, 3)) = "SPA" Then
    ...
    If val(rData) > 0 And val(rData) < UBound(SpawnList) + 1 Then _
          ...
          Call LogGM("EDITADOS", UserList(UserIndex).name & " Sumoneo un " & SpawnList(val(rData)).NpcName, False)
...
If UCase(Left(rData, 4)) = "/CI " Then
    ...
    Call SendData(SendTarget.ToAdmins, 0, 0, "||" & UserList(UserIndex).name & " Tiró un/una " & ObjData(Objeto.ObjIndex).name & " en el mapa " & UserList(UserIndex).Pos.Map & FONTTYPE_GUILD)
    Exit Sub
End If
...
If UCase$(Left$(rData, 5)) = "/ACC " Then
   ...
   Call LogGM("EDITADOS", UserList(UserIndex).name & " Sumoneo un " & Npclist(val(rData)).name & " en mapa " & UserList(UserIndex).Pos.Map, False)
   Exit Sub
End If

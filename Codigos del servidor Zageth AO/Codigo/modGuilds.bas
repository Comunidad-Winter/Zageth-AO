...
Dim DummyString     As String
    ...
    Call QuitarObjetos(778, 10, FundadorIndex)
...
Public Function PuedeFundarUnClan(ByVal UserIndex As Integer, ByVal Alineacion As ALINEACION_GUILD, ByRef refError As String) As Boolean
    ...
    PuedeFundarUnClan = False
        If TieneObjetos(778, 10, UserIndex) = False Then
...
On Error GoTo errh
    Call SendData(SendTarget.ToAll, 0, 0, "||Zageth AO> Revisando elecciones" & FONTTYPE_SERVER)
    For i = 1 To CANTIDADDECLANES
...
proximo:
    Next i
    Call SendData(SendTarget.ToAll, 0, 0, "||Zageth AO> Elecciones revisadas" & FONTTYPE_SERVER)
Exit Sub
errh:

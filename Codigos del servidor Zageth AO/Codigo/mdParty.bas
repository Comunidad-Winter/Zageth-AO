                    ...
                    If UserList(NewMember).PartySolicitud = PI Then
                        If Parties(PI).PuedeEntrar(NewMember, razon) Then
                            ...
                            Else
                                ...
                                Call SendData(SendTarget.ToAdmins, Leader, 0, "|| Zageth AO> CATASTROFE EN PARTIES, NUEVOMIEMBRO DIO FALSE! :S " & FONTTYPE_PARTY)
                            End If
                        Else
                            'no debe entrar
...
If Not PARTY_EXPERIENCIAPORGOLPE Then
    ...
    Call SendData(SendTarget.ToAll, 0, 0, "||Zageth AO> Distribuyendo experiencia en parties." & FONTTYPE_SERVER)
    For i = 1 To MAX_PARTIES
        If Not Parties(i) Is Nothing Then
            Call Parties(i).FlushExperiencia
        End If
    Next i
    Call SendData(SendTarget.ToAll, 0, 0, "||Zageth AO> Experiencia distribuida." & FONTTYPE_SERVER)
    Call SendData(SendTarget.ToAll, 0, 0, "BKW")
    haciendoBK = False
